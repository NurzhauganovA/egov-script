...
def main(count_loop, choice_licensor_arg, full_name_representative_arg, phone_number_arg, location_land_plot_arg,
...
def auth_with_eds(driver, choice_licensor_arg, full_name_representative_arg, phone_number_arg, location_land_plot_arg, requested_right_use_arg, area_arg, purpose_use_land_plot_arg, schema_plan_land_plot_arg):
    ...
    try:
        send_auth_request(sessionID, timeTicket, choice_licensor_arg, full_name_representative_arg, phone_number_arg, location_land_plot_arg, requested_right_use_arg, area_arg, purpose_use_land_plot_arg, schema_plan_land_plot_arg)
        return True
    except Exception as e:
        print(e)
        return False
...
class EgovForm(QMainWindow):
    ...
    def get_data_from_selected_file(self, file_path):
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            choice_licensor, full_name_representative, phone_number, location_land_plot, requested_right_use, area, purpose_use_land_plot, order_template = row[:8]
            ...
            data.append(
                {
                    ...
                    "location_land_plot": location_land_plot,
                    "requested_right_use": requested_right_use,
                    "area": area,
                    "purpose_use_land_plot": purpose_use_land_plot,
    ...
    def start_program(self, data):
        ...
        while count_loop < len(data):
            try:
                ...
                main(count_loop, row["choice_licensor"], row["full_name_representative"], row["phone_number"],
                     row["location_land_plot"], row["requested_right_use"], row["area"], row["purpose_use_land_plot"],
                     row["order_template"])
