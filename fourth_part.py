...
def choice_licensor(object_name):
    append_to_context('choice_licensor',
                      get_choice_licensor_by_object_name(object_name))
...
def set_name_object(text_lines):
    ...
    if start_idx is not None and end_idx is not None:
        object_name = ' '.join(
            line.strip() for line in text_lines[start_idx:end_idx]).replace('\xa0', ' ')
...
def main(path):
    ...
    try:
        ...
        # enter the path of the download file
        ...
        button_select_certificate = driver.find_element(
            By.ID, "buttonSelectCert")
...
def elicense_new_tab(driver, choice_licensor_arg, full_name_representative_arg, phone_number_arg, full_name_object_rus_arg,
    ...
    try:
        ...
        # click 'Строительство'
        driver.find_element(By.ID, "new-service-button-af-14").click()
        driver.implicitly_wait(30)
        # click 'Предоставление исходных материалов при разработке'
        driver.find_elements(
            By.CLASS_NAME, "new-detail-single")[0].find_elements(By.TAG_NAME, "a")[7].click()
        ...
        if city:
            for oblast_item in oblast_list:
                oblast_name = oblast_item.find_element(
                ...
                if city in oblast_name:
                    ...
                    oblast_item.find_element(By.TAG_NAME, "li").find_element(
            ...
            if oblast in oblast_name:
                ...
                if sel_okrug:
                    for sel_okrug_item in sel_okrug_list:
                        sel_okrug_name = sel_okrug_item.find_element(
                            By.TAG_NAME, "span").text
                        if sel_okrug in sel_okrug_name:
                            clickable_obj = sel_okrug_item.find_element(
                                By.TAG_NAME, "span")
                ...
                if region:
                    for region_item in sel_okrug_list:
                        ...
                        if region in region_name:
                            clickable_obj = region_item.find_element(
                                By.TAG_NAME, "span")
                ...
                if city:
                    for city_item in city_list:
                        city_name = city_item.find_element(
                            By.TAG_NAME, "span").text
                        if city in city_name:
                            clickable_obj = city_item.find_element(
                                By.TAG_NAME, "span")
                            if len(found_list) > 1 and clickable_obj.text in [item.text for item in found_list]:
                                found_list = [clickable_obj]
                            else:
...
def create_order(driver, full_name_representative_arg, phone_number_arg, full_name_object_rus_arg,
    ...
    try:
        ...
        driver.implicitly_wait(100)
        driver.find_element(By.ID, "toolbar-1016-targetEl").find_elements(
            By.TAG_NAME, "div")[0].click()  # button "Save"

        driver.implicitly_wait(100)

        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.ID, "button-1005-btnInnerEl"))  # button "OK"
        ).click()

        driver.implicitly_wait(100)

        driver.find_element(By.ID, "toolbar-1016-targetEl").find_elements(
            By.TAG_NAME, "div")[1].click()  # button "Next"
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, "/html/body/div/div/div/div[3]/div/form/div/table/tbody/tr[2]/td[1]/a"))
        ).click()
        ...
        close_table = driver.find_elements(
        ...
        file_path = os.path.join(
            absolute_path, 'files', number_orders + '_ru.pdf')
